...
# map by core 
mpirun -np 2 --report-bindings ./IMB-MP1 PingPong 2>/dev/null | grep -v ^# | grep -v -e '^$' |  tr -s ' '| sed 's/^[ \t]+*//g' | sed 's/[ \t]+*/,/g' > core_ucx.csv

mpirun -np 2 -mca pml ob1 --mca btl self,tcp  --report-bindings ./IMB-MPI1 PingPong 2>/dev/null  grep -v ^# | grep -v -e '^$' |  tr -s ' '| sed 's/^[ \t]+*//g' | sed 's/[ \t]+*/,/g' > core_tcp.csv 
